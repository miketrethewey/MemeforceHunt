...
import static io.github.alttpj.memeforcehunt.common.value.Palette.BLUE;
import static io.github.alttpj.memeforcehunt.common.value.Palette.GREEN;
import static io.github.alttpj.memeforcehunt.common.value.Palette.RED;
...
public enum Skin {
	...
	private Skin(final String name, final String fileName, final Palette p) {
		this.name = name;
		this.fileName = fileName;
		this.p = p;
		final InputStream temp;
		final ArrayList<Byte> btemp = new ArrayList<Byte>();
		...
		final int l = btemp.size();
		int i = 0;
		this.data = new byte[l];
		for (final Byte b : btemp) {
			this.data[i++] = b;
		}
		...
		this.preview = new BufferedImage(32, 32, BufferedImage.TYPE_4BYTE_ABGR);
		final Graphics2D g = this.preview.createGraphics();

		g.scale(2, 2);
		g.drawImage(itemp, 0, 0, null);
		g.dispose();
		this.ico = new ImageIcon(this.preview);
		this.icoSmall = new ImageIcon(itemp);
	}
